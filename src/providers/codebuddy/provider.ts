...
import type {
  ...
  CodeBuddyModelPolicyMode,
  CodeBuddyModelTier,
  CodeBuddyModelTierName
} from "../../config/config.js";

async function resolveCodeBuddyCommand(): Promise<string | undefined> {
  const configured = process.env.SYSTWO_CODEBUDDY_BIN;
  if (configured && (await commandExists(configured))) {
    return configured;
  }
  if (await commandExists("codebuddy")) {
    return "codebuddy";
  }
  if (await commandExists("cbc")) {
    return "cbc";
  }
  return undefined;
}

export const codeBuddyProvider: Provider = {
  id: "codebuddy",
  ...
  async estimateUsage(input: TaskBrief) {
    return {
      ...
      basis: "Heuristic estimate; CodeBuddy actual usage is collected only when exposed by its CLI."
    };
  },
  async doctor() {
    ...
    return {
      ok: Boolean(command),
      message: command
        ? `CodeBuddy CLI was found as "${command}". SysTwo will invoke it with print/headless mode in a temporary worktree. Model policy is resolved from SysTwo config at run time; doctor does not make model calls.`
        : "CodeBuddy CLI was not found as codebuddy or cbc; mock provider remains available."
    };
  },
  async runTask(input: TaskBrief, options: RunOptions): Promise<TaskResult> {
    ...
    if (!command) {
      return {
        status: "failed",
        summary: "CodeBuddy CLI was not found on PATH.",
        provider: "codebuddy",
        traceId: options.traceId,
        worktreePath: options.worktreePath,
        usage: { estimated, actual: unavailableActualUsage() },
        riskNotes: ["Install CodeBuddy or use provider=mock for the zero-config demo."]
      };
    }

    const modelResolution = resolveCodeBuddyModelArgs(input, options.config?.providers.codebuddy.modelPolicy);
    ...
    const args = buildCodeBuddyArgs(input, options.mode, modelResolution.args);
    const result = await runCommand(command, args, {
      cwd,
      allowFailure: true,
      timeoutMs: options.timeoutMs ?? Number(process.env.SYSTWO_CODEBUDDY_TIMEOUT_MS ?? "120000")
    });

    const parsedOutput = parseCodeBuddyJsonOutput(result.stdout);
    ...
    const permissionRiskNotes =
      parsedOutput && parsedOutput.permissionDenials.length > 0
        ? [`CodeBuddy reported permission denials: ${parsedOutput.permissionDenials.slice(0, 3).join("; ")}`]
        : [];
    ...
    return {
      ...
      summary:
        result.exitCode === 0
          ? summarizeCodeBuddyOutput(resultText, options.mode)
          : summarizeCodeBuddyFailure(result.stderr || resultText || result.stdout, modelResolution),
      provider: "codebuddy",
      traceId: options.traceId,
      worktreePath: options.worktreePath,
...
export type CodeBuddyModelArgsResolution =
  | {
      ok: true;
      args: string[];
      mode: CodeBuddyModelPolicyMode;
      tierName?: CodeBuddyModelTierName;
      model?: string;
...
const CODEBUDDY_EFFORTS = new Set<string>(["minimal", "low", "medium", "high", "xhigh", "max"]);
const AUTO_MODEL_POLICY: CodeBuddyModelPolicy = { mode: "auto", tiers: {} };
...
export function resolveCodeBuddyModelArgs(
  ...
  if (!MODEL_POLICY_MODES.has(String(policy.mode))) {
    return invalidModelPolicy(`Invalid CodeBuddy model policy mode: ${String(policy.mode)}.`);
  }

  const mode = policy.mode as CodeBuddyModelPolicyMode;
  if (mode === "auto") {
    return {
      ok: true,
      args: [],
      mode,
      riskNote: "CodeBuddy model policy mode=auto; SysTwo did not pass --model."
    };
  }

  if (!isRecord(policy.tiers)) {
    return invalidModelPolicy("Invalid CodeBuddy model policy tiers: expected an object.");
  }
  ...
  if (!isConfiguredTier(rawTier)) {
    if (mode === "hybrid") {
      return {
        ...
        mode,
        tierName,
        riskNote: `CodeBuddy model policy mode=hybrid; no ${tierName} tier is configured, so SysTwo did not pass --model.`
      };
    }
    return invalidModelPolicy(`CodeBuddy model policy mode=manual requires a configured ${tierName} tier.`);
  }

  if (!isRecord(rawTier)) {
    return invalidModelPolicy(`Invalid CodeBuddy model policy tier "${tierName}": expected an object.`);
  }

  const tier = rawTier as CodeBuddyModelTier;
  const validationError = validateCodeBuddyModelTier(tierName, tier, mode);
  if (validationError) {
  ...
  return {
    ...
    effort: tier.effort,
    riskNote: `CodeBuddy model policy mode=${mode}; ${tierName} tier selected${tier.model ? ` model=${tier.model}` : " without --model"}.`
  };
}
...
function modelTierForTask(input: TaskBrief): CodeBuddyModelTierName {
...
function validateCodeBuddyModelTier(
  tierName: CodeBuddyModelTierName,
  tier: CodeBuddyModelTier,
  mode: CodeBuddyModelPolicyMode
): string | undefined {
  if (mode === "manual" && !isNonEmptyString(tier.model)) {
    return `CodeBuddy model policy mode=manual requires ${tierName}.model.`;
  }
  if (tier.model !== undefined && !isNonEmptyString(tier.model)) {
    return `Invalid CodeBuddy model policy tier "${tierName}": model must be a non-empty string.`;
  }
  if (tier.fallbackModel !== undefined && !isNonEmptyString(tier.fallbackModel)) {
    return `Invalid CodeBuddy model policy tier "${tierName}": fallbackModel must be a non-empty string.`;
  }
  if (tier.effort !== undefined && !CODEBUDDY_EFFORTS.has(String(tier.effort))) {
    return `Invalid CodeBuddy model policy tier "${tierName}": effort must be one of minimal, low, medium, high, xhigh, max.`;
  }
  return undefined;
}

function invalidModelPolicy(message: string): CodeBuddyModelArgsResolution {
  return {
    ok: false,
    summary: `${message} SysTwo did not fall back to auto because an explicit CodeBuddy model policy was configured. Check systwo.yaml, ~/.config/systwo/config.yaml, CodeBuddy models.json, availableModels, or the CodeBuddy CLI supported model list.`,
    riskNotes: [
      "CodeBuddy model policy configuration is invalid.",
...
function buildPrompt(input: TaskBrief, mode: RunOptions["mode"]): string {
  return [
    "You are running as a bounded runner behind SysTwo.",
    "",
    `Goal: ${input.goal}`,
...
type ParsedCodeBuddyOutput = {
  resultText?: string;
  actualUsage?: UsageActual;
  permissionDenials: string[];
};

export function parseCodeBuddyJsonOutput(stdout: string): ParsedCodeBuddyOutput | undefined {
  try {
    ...
    for (const event of events) {
      ...
      if (event.type === "result") {
        ...
        if (isRecord(event.usage)) {
          actualUsage = actualUsage ?? usageFromCodeBuddyObject(event.usage);
        }
      ...
      if (event.type === "message" && event.role === "assistant" && !resultText) {
        resultText = assistantTextFromMessageEvent(event);
      }

      if (isRecord(event.providerData)) {
        if (isRecord(event.providerData.usage)) {
          actualUsage = actualUsage ?? usageFromCodeBuddyObject(event.providerData.usage);
        }
        if (isRecord(event.providerData.rawUsage)) {
          actualUsage = actualUsage ?? usageFromCodeBuddyObject(event.providerData.rawUsage);
        }
      }
    }
...
function assistantTextFromMessageEvent(event: Record<string, unknown>): string | undefined {
  if (!Array.isArray(event.content)) {
    return undefined;
  }
  const chunks = event.content
    .filter(isRecord)
    .map((chunk) => (chunk.type === "output_text" && typeof chunk.text === "string" ? chunk.text : ""))
    .filter(Boolean);
  return chunks.length > 0 ? chunks.join("\n") : undefined;
}

function usageFromCodeBuddyObject(usage: Record<string, unknown>): UsageActual | undefined {
...
function summarizeCodeBuddyOutput(stdout: string, mode: RunOptions["mode"]): string {
  const trimmed = stdout.trim();
  if (!trimmed) {
    if (mode === "patch_only") {
      return "CodeBuddy completed but returned no patch proposal for patch_only mode.";
    }
    if (mode === "temp_worktree") {
      return "CodeBuddy completed. SysTwo will convert worktree changes into diff evidence.";
    }
    return "CodeBuddy completed without returning result text.";
  }
  if (mode === "patch_only") {
...
function summarizeCodeBuddyFailure(stdoutOrStderr: string, modelResolution: Extract<CodeBuddyModelArgsResolution, { ok: true }>): string {
  ...
  const policyContext =
    modelResolution.mode === "auto"
      ? "CodeBuddy model policy mode=auto."
      : `CodeBuddy model policy mode=${modelResolution.mode}, tier=${modelResolution.tierName}${configuredModel}${configuredFallback}.`;
  const guidance =
    modelResolution.mode === "auto"
      ? ""
  ...
  const message = `CodeBuddy failed. ${policyContext}${guidance}${details ? ` Output: ${details}` : ""}`;
